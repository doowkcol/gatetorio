...
class GateController:
    def __init__(self, config_file='/home/doowkcol/Gatetorio_Code/gate_config.json'):
        ...
        input_config = {
            ...
            'input_sample_rate': 0.01  # 100Hz sampling (ADS1115 max: ~107Hz for 8 channels)
        }
    ...
    def _evaluate_commands(self, now):
        ...
        if not self.shared['cmd_open_active'] and not self.shared['timed_open_active']:
            if self.shared['state'] in ['OPEN', 'UNKNOWN'] and self.auto_close_enabled and not self.shared['auto_close_active']:
